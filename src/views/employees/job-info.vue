...
import { getEmploeesJobInfoAPI, saveEmploeesJobInfoAPI } from '@/api/employees'
...
export default {
  ...
  created() {
    this.getEmployeesJob()
  },
  methods: {
    async getEmployeesJob() {
      this.formData = await getEmploeesJobInfoAPI(this.userId)
    },
    async saveEmployeesJob() {
      console.log(111)
      try {
        await saveEmploeesJobInfoAPI(this.formData)
